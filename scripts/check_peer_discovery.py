...
def main() -> int:
    ...
    if not raw.exists() or not dd.exists():
        print("[check_peer_discovery] FAIL: missing raw or dedup file", file=sys.stderr)
        return 1
    try:
        deduped = json.loads(dd.read_text(encoding="utf-8"))
    except Exception as e:
        print(f"[check_peer_discovery] FAIL: bad JSON: {e}", file=sys.stderr)
        return 1
    if not isinstance(deduped, list) or len(deduped) < 16:
        print(
            f"[check_peer_discovery] FAIL: need >= 16 peers, got {len(deduped) if isinstance(deduped, list) else 'n/a'}",
            file=sys.stderr,
        )
        return 1
    arms_seen = set()
    ...
    if len(arms_seen) < 2:
        print(
            f"[check_peer_discovery] FAIL: peer evidence came from only {len(arms_seen)} arm(s); need >= 2",
            file=sys.stderr,
        )
        return 1
    miss_url = [p for p in deduped if not p.get("url")]
    if miss_url:
        print(f"[check_peer_discovery] FAIL: {len(miss_url)} peer(s) without url", file=sys.stderr)
        return 1
    print(f"[check_peer_discovery] ok ({len(deduped)} peers, {len(arms_seen)} arms)")
    return 0
