...
def main() -> int:
    ...
    if not p.exists():
        print(f"[check_interview_guide] FAIL: missing {p}", file=sys.stderr)
        return 1
    t = p.read_text(encoding="utf-8")
    miss = [s for s in REQUIRED_SECTIONS if s.lower() not in t.lower()]
    if miss:
        print(f"[check_interview_guide] FAIL: missing sections {miss}", file=sys.stderr)
        return 1
    if "push" not in t.lower() or "pull" not in t.lower():
        print("[check_interview_guide] FAIL: force mapping (push/pull) missing", file=sys.stderr)
        return 1
    print("[check_interview_guide] ok")
    return 0
