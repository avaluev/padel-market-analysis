...
REQUIRED = {
    ...
    "aura_stage": (lambda v: v in {"awareness", "understanding", "readiness", "action"}, "one of A/U/R/A"),
    ...
    "kill_experiments": (lambda v: isinstance(v, list) and len(v) >= 3, "list with >=3 experiments"),
}
...
def main() -> int:
    ...
    if not p.exists():
        print(f"[check_blueprint] FAIL: missing {p}", file=sys.stderr); return 1
    try:
        data = json.loads(p.read_text(encoding="utf-8"))
    except json.JSONDecodeError as e:
        print(f"[check_blueprint] FAIL: invalid JSON: {e}", file=sys.stderr); return 1
    fail = 0
    for k, (validator, msg) in REQUIRED.items():
        if k not in data:
            print(f"[check_blueprint] FAIL: missing key '{k}'", file=sys.stderr); fail += 1; continue
        if not validator(data[k]):
            print(f"[check_blueprint] FAIL: '{k}' must be {msg}", file=sys.stderr); fail += 1
