...
"""
import argparse, json, pathlib, re, sys, html as htmllib
    ...
    def repl(m):
        ...
        for it in items:
            ...
            for var_m in re.finditer(r"\{\{this\.([\w\.]+)\}\}", body):
                sub = sub.replace(var_m.group(0), htmllib.escape(str(get(it, var_m.group(1)) or "")))
            sub = sub.replace("{{this}}", htmllib.escape(str(it) if not isinstance(it, (dict,list)) else ""))
            out.append(sub)
        return "".join(out)
...
def main() -> int:
    ...
    print(f"[render_template] wrote {out} ({out.stat().st_size} bytes)"); return 0
