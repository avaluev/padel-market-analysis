...
PATTERNS = [
    ...
    re.compile(
        r"\bby\s+(Q[1-4]|January|February|March|April|May|June|July|August|September|October|November|December|H[12]\s+\d{4})\b",
        re.I,
    ),
...
def main() -> int:
    ...
    if not rep.exists():
        print("[check_no_future_tense] no report yet")
        return 0
    fail = 0
    for f in rep.rglob("*"):
        ...
        for pat in PATTERNS:
            for m in pat.finditer(text):
                print(f"[check_no_future_tense] FAIL: {f}: {m.group(0)!r}", file=sys.stderr)
                fail += 1
