...
def main() -> int:
    ...
    if not p.exists():
        print(f"[check_aura] FAIL: missing {p}", file=sys.stderr)
        return 1
    try:
        d = json.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        print(f"[check_aura] FAIL: {e}", file=sys.stderr)
        return 1
    f = 0
    if d.get("stage") not in ALLOWED:
        print("[check_aura] FAIL: bad stage", file=sys.stderr)
        f += 1
    for k, m in [("entry_signals", 2), ("exit_signals", 2), ("kill_experiments", 3), ("risks", 2)]:
        v = d.get(k)
        if not isinstance(v, list) or len(v) < m:
            print(f"[check_aura] FAIL: '{k}' needs >= {m} items", file=sys.stderr)
            f += 1
    if not isinstance(d.get("rationale"), str) or len(d["rationale"]) < 200:
        print("[check_aura] FAIL: rationale too short (<200 chars)", file=sys.stderr)
        f += 1
