...
def main() -> int:
    ...
    if not p.exists():
        print(f"[check_canonical_brief] FAIL: missing {p}", file=sys.stderr)
        return 1
    try:
        d = json.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        print(f"[check_canonical_brief] FAIL: {e}", file=sys.stderr)
        return 1
    f = 0
    for k in REQUIRED_SECTIONS:
        if k not in d:
            print(f"[check_canonical_brief] FAIL: missing section '{k}'", file=sys.stderr)
            f += 1
    usp = (d.get("usp") or "").strip()
    if not usp:
        print("[check_canonical_brief] FAIL: usp missing", file=sys.stderr)
        f += 1
    else:
        n = len(re.findall(r"\S+", usp))
        if n > 30:
            print(f"[check_canonical_brief] FAIL: usp is {n} words (max 30)", file=sys.stderr)
            f += 1
        for pat in BANNED_USP_TOKENS:
            if re.search(pat, usp, re.IGNORECASE):
                print(
                    f"[check_canonical_brief] FAIL: usp uses banned token /{pat}/", file=sys.stderr
                )
                f += 1
    diff = d.get("differentiation_table") or []
    if len(diff) < 8:
        print(
            f"[check_canonical_brief] FAIL: differentiation_table has {len(diff)} rows (need >= 8)",
            file=sys.stderr,
        )
        f += 1
