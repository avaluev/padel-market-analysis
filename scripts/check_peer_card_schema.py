...
def main() -> int:
    ...
    if not d.is_dir():
        print(f"[check_peer_card_schema] FAIL: missing {d}", file=sys.stderr)
        return 1
    files = list(d.glob("*.json"))
    if len(files) < 8:
        print(
            f"[check_peer_card_schema] FAIL: need >= 8 peer cards, got {len(files)}",
            file=sys.stderr,
        )
        return 1
    f = 0
    for fp in files:
        try:
            ...
            print(f"[check_peer_card_schema] FAIL: {fp.name}: {e}", file=sys.stderr)
            f += 1
            continue
        for k in REQUIRED:
            if k not in c:
                print(f"[check_peer_card_schema] FAIL: {fp.name} missing '{k}'", file=sys.stderr)
                f += 1
        m = (c.get("moat_class") or "").lower()
        if m in FORBIDDEN_MOAT:
            print(
                f"[check_peer_card_schema] FAIL: {fp.name} uses forbidden moat '{m}'",
                file=sys.stderr,
            )
            f += 1
        if m not in ALLOWED_MOAT and m not in FORBIDDEN_MOAT:
            print(f"[check_peer_card_schema] FAIL: {fp.name} unknown moat '{m}'", file=sys.stderr)
            f += 1
